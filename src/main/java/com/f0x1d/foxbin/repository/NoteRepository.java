...
public class NoteRepository {
    ...
    private FoxBinUser rootUser() {
        return ObjectBox.get().callInReadTx(() -> mRootUser == null ? mRootUser = DBUtils.findFirst(
                DBUtils
                        .boxForUsers()
                        .query()
    ...
    public void checkExpiration() {
        ObjectBox.get().runInTx(() -> {
            ...
            List<FoxBinNote> notesToRemove = new ArrayList<>();
            ...
            for (FoxBinNote expiringNote : expiringNotes) {
                if (expiringNote.getDate().getTime() + expiringNote.getDeleteAfter() <= System.currentTimeMillis())
                    notesToRemove.add(expiringNote);
            }

            noteBox.remove(notesToRemove);
        });
    }
}
